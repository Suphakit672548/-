...
def edit_book(book_id, title, author):
    ...
    return True, "แก้ไขข้อมูลหนังสือแล้ว"


def remove_book(book_id):
    model.delete_book(book_id)
    return True, "ลบหนังสือแล้ว"
...
def validate_member(member_code, name, email, ignore_id=None):
    errors = []
    ...
    if email.strip() and not re.match(EMAIL_PATTERN, email):
        errors.append("อีเมลไม่ถูกต้อง")

    if model.member_code_exists(member_code.strip()):
        errors.append(f"รหัส {member_code} มีในระบบแล้ว")

    if email.strip() and model.email_exists(email.strip()):
        errors.append(f"อีเมล {email} ถูกใช้แล้ว")

    return errors


def create_member(member_code, name, gender, email, phone):
    errors = validate_member(member_code, name, email)
    ...
    return True, "สมัครสมาชิกเรียบร้อย"
...
def update_member(
    ...
    if member_code != old_member_code and model.member_code_exists(member_code):
        errors.append("รหัสสมาชิกซ้ำ")

    if email and email != old_email and model.email_exists(email):
        errors.append("อีเมลซ้ำ")
    ...
    return True, "แก้ไขข้อมูลสมาชิกแล้ว"


def remove_member(member_id):
    model.delete_member(member_id)
    return True, "ลบสมาชิกแล้ว"
