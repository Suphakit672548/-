...
def add_member(member_code, name, gender, email, phone):
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?)
        """,
        (member_code, name, gender, email, phone)
    )
    conn.commit()
    conn.close()
...
def update_member(
    ...
    c.execute(
        ...
        WHERE id=?
        """,
        (member_code, name, gender, email, phone, int(is_active), member_id)
    )
    conn.commit()
    conn.close()


def member_code_exists(code):
    conn = get_connection()
    c = conn.cursor()
    c.execute("SELECT id FROM members WHERE member_code=?", (code,))
